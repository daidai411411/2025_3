...
export const useAddSand = () => {
  ...
  return (): number => {
    if (userData) {
      const newSand =
        userData.sand + 30 * Math.pow(2.2, userData.tool_level - 1); //この辺用改変
  ...
  return (index: number): void => {
    if (userData && facility) {
      const fee = Math.round(
        facility[index].cost *
          facility[index].magnification *
          Math.log(2 + (userData.facility[index] - 1)) **
            (userData.facility[index] - 1)
      );
      if (userData.money < fee) {
        alert("お金が足りません");
        return;
        ...
        if (prev) {
          return prev.map((fac, index) => {
            if (userData.facility[index] >= 1) {
              const increment =
                fac.efficiency *
                userData.facility[index] *
                ((userData.elevation / 500) * 9 + 1) *
                deltaTime;
